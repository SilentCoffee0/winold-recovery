...
- `GameSaves.rules.json` — Saved Games, My Games, Steam userdata, Steam library `steamapps\common` save folders, Unreal `Saved\SaveGames`, GOG Galaxy Applications, `.minecraft`, Epic, Ubisoft, EA/Origin, Battle.net/Blizzard, Riot Games, LocalLow publisher folders, Steam-compatible AppData save folders
- `Sensitive.rules.json` — keys, `.env`, `.yarnrc.yml`, certs next to a key, cloud CLI credentials, GPG, browser Login Data/Cookies/key4/logins, NTUSER.DAT, DPAPI Protect, Credential vault, password-manager exports (`.1pux`, Bitwarden/LastPass export names, 1Password Emergency Kit)

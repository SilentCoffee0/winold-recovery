...
WSL 2 distros live in an `ext4.vhdx`. WinOld Recovery copies that file to `AppData\Local\wsl\recovered\<name>\` and checks the `vhdxfile` header. It never runs `wsl.exe` against Windows.old.

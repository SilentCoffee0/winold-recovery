...
Firefox is different: `logins.json` (or `logins.db` on newer profiles) and `key4.db` are restored together into a new profile. The card reports whether a Primary Password is set, not set, or could not be determined from `key4.db` metadata. If one is set, Firefox will ask for it after restore. The tool never exports those passwords as text. Verify requires `key4.db` to sit next to `logins.json` or `logins.db`.

...
- Published `--restore source dest --report` copies a tree without the WPF window (and resumes the newest interrupted copy session). `tools/run-kill-published-copy.ps1` is the elevated kill-and-resume probe.
- Published `--restore source dest --report` copies a tree without the WPF window (and resumes the newest interrupted copy session). `tools/run-kill-published-copy.ps1` is the elevated kill-and-resume probe.

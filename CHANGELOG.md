...
- Looking for apps reuses the NTUSER.DAT copy already made during profile detection (Anki/WSL no longer copy the hive again). Anki detect reads note counts without `PRAGMA integrity_check`. Outlook/KeePass index queries stay inside Documents/Desktop/Downloads (and Outlook AppData). WSL disks are found by the `ext4.vhdx`/`docker_data.vhdx` name index instead of scanning AppData Local for every `.vhdx`.

...
- After a scan, Git/KeePass/Obsidian/Outlook, Anki Start-menu `.lnk`, Windows Terminal `settings.json`, WSL `ext4.vhdx`/`docker_data.vhdx`, and Steam library `steamapps\common` save folders detect use the node index instead of walking the profile again. Anki media file counts also come from the index (excluding `media.trash`) instead of walking `collection.media`. Anki still reads `-b` bytes from those shortcuts. Detect without a walker checkpoint still walks the disk (unit tests).
- Terminal and Obsidian L3 verify require dest file lengths to match the source, so an empty or truncated restore fails.

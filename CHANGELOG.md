...
- FixtureGen creates git repositories in the eight spec states (clean+pushed, uncommitted, untracked, unpushed, local-only branch, stash, no remote, worktree). Git L3 verify compares copied working-tree file lengths to the source and treats a worktree `.git` file as a `gitdir:` pointer.

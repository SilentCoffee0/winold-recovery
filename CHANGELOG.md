...
- `tools/run-kill-published-copy.ps1` waits until destination files exist, refuses a pass if the copy already finished, kills by PID (never `/IM`), and uses UAC `taskkill` when Medium IL cannot `Stop-Process` an elevated EXE.

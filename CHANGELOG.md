...
- After a scan, Git/KeePass/Obsidian/Outlook, Anki Start-menu `.lnk`, Windows Terminal `settings.json`, and WSL `ext4.vhdx`/`docker_data.vhdx` detect use the node index instead of walking the profile again. Anki still reads `-b` bytes from those shortcuts. Detect without a walker checkpoint still walks the disk (unit tests).
...
- SSH L3 verify runs `ssh -G localhost` against the restored `.ssh` folder when ssh.exe answers. Missing ssh skips the process check. Game-save cards also restore `Program Files (x86)\Steam\userdata` into `Saved Games\Steam userdata`, Steam library `steamapps\common` save folders, and LocalLow publisher / Steam-compatible folders.

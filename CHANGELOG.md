...
- Elevated FixtureGen self-check enables backup privilege, lists deny-ACL / orphan-SID children with `FileSystemEnumerator`, reads the deny-ACL file through `BackupFile.OpenRead`, and fails if a source `FileSystemWatcher` records writes.

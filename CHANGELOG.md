...
- After a scan, Git/KeePass/Obsidian/Outlook, Anki Start-menu `.lnk`, Windows Terminal `settings.json`, WSL `ext4.vhdx`/`docker_data.vhdx`, Steam library `steamapps\common` save folders, and Zotero/Calibre/Joplin/Logseq libraries detect use the node index instead of walking the profile again. Joplin cards after a checkpoint include folders named `Joplin` that contain `database.sqlite` (not only `%APPDATA%\Joplin`). Anki media, `.colpkg` backup, and `addons21` counts also come from the index (excluding `media.trash`) instead of walking those folders. Anki still reads `-b` bytes from those shortcuts. Detect without a walker checkpoint still walks the disk (unit tests).

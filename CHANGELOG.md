...
- GPG L3 verify runs `gpg --list-secret-keys` with `GNUPGHOME` on the restored ring. Missing `gpg.exe` is skipped; a non-zero exit fails verify. Restore no longer invokes gpg during copy.

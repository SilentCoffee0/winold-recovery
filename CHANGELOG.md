...
- WSL registration lists `wsl -l -v` first and imports as `<name>-recovered` when that distro name is already taken. Preview keep-both copy names the `(from Windows.old)` suffix.

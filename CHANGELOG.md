...
- Looking for apps no longer copies Chrome History / Edge Web Data / Firefox places, and no longer reads Chrome Bookmarks JSON, walks Extensions, or parses SNSS session files during detect. Those exports are built at Plan. After a walker checkpoint, Chrome/Edge extension counts come from immediate child directories in the node index instead of listing `Extensions`. After that, `Bookmarks` / `History` / `Web Data` presence also comes from the index (no `DirectoryExists` on User Data and no `FileExists` on those three files). Firefox detect no longer decodes sessionstore or extensions.json into card facts. Apps & folders / All files stay disabled until the scan finishes.
...
- Game-save, Outlook PST, KeePass vault, library catalog, Terminal, Obsidian, GPG, SSH, VS Code, Thunderbird, Firefox transplant, Chromium transplant copies, Git copied files, WSL VHDX, Syncthing identity files, and Anki L3 verify require dest file lengths to match the source, so an empty or truncated restore fails.
- A finished scan writes `last-scan.json`. The next launch offers Open last scan so the tree does not have to be walked again.
- Decide lists Apps first, then Personal folders. Restore marks only the selection; everything else stays Undecided. Later keeps the item Undecided so it can be restored later. Leave Behind still does not delete. The Cards/Files toggle is labeled Apps & folders / All files. The Scan source picker stays on the Scan tab. The All files tree shows ▸/▾ fold controls, indent, size, and percent of parent.
...
- After a scan, Git/KeePass/Obsidian/Outlook plus KeePass sibling `.keyx`/`.key`, Anki Start-menu `.lnk` and `collection.anki2` profiles then `collection.anki2-wal`, Windows Terminal `settings.json`, WSL `ext4.vhdx`/`docker_data.vhdx` plus WSL 1 `lxss`, Steam library `steamapps\common` save folders then known Saved Games / My Games / launcher and LocalLow folders, Zotero/Calibre/Joplin/Logseq libraries, Syncthing homes then identity `cert.pem`/`key.pem`/`config.xml` plus `https-cert.pem`/`https-key.pem` and `gui`, GPG keyrings then `pubring.kbx`/`secring.gpg`/`private-keys-v1.d` presence, Firefox `places.sqlite`/`prefs.js` profile folders then allow-listed files, `sessionstore.jsonlz4`/`sessionstore-backups`, transplant folders, and `profiles.ini`/`installs.ini` StoreID, Thunderbird `prefs.js` profile folders then allow-listed files plus `Mail`/`ImapMail`, Chrome/Edge `Bookmarks` profile folders plus `Session_*`/`Tabs_*` presence then `Local State`/`Last Version` labels, VS Code/VSCodium/Cursor User `settings.json`/`keybindings.json`/`tasks.json` plus snippets/profiles/`extensions.json`, SSH `.ssh` plus `ProgramData\ssh`, and Git `.gitconfig` / `.config\git\config` / `.git-credentials` / ignore sidecars detect use the node index instead of walking those folders again. Joplin cards after a checkpoint include folders named `Joplin` that contain `database.sqlite` (not only `%APPDATA%\Joplin`). Syncthing cards after a checkpoint include custom `--home` folders that contain `cert.pem`/`config.xml`/`key.pem` (not only Local/Roaming/ProgramData/SYSTEM). GPG cards after a checkpoint include folders that contain `pubring.kbx`, `secring.gpg`, or `private-keys-v1.d` (not only `%APPDATA%\gnupg` and `\.gnupg`). Decide also has **Add a Syncthing folder…** for `--home`/`STHOMEDIR` locations the walk missed. Anki media, `.colpkg` backup, and `addons21` counts also come from the index (excluding `media.trash`) instead of walking those folders. Anki still reads `-b` bytes from those shortcuts. Detect without a walker checkpoint still walks the disk (unit tests).
- FixtureGen creates git repositories in the eight spec states (clean+pushed, uncommitted, untracked, unpushed, local-only branch, stash, no remote, worktree). Git L3 verify compares copied working-tree file lengths to the source and treats a worktree `.git` file as a `gitdir:` pointer. Recipe shells now include real-format Chrome Bookmarks/`Local State`/History/SNSS, Firefox `places.sqlite`/mozLz4 sessionstore, a Syncthing identity whose device ID is derived from a self-signed cert, generated OpenSSH keys (unencrypted and `aes256-ctr`), and an empty Anki `collection.anki2` schema.
- Anki L3 verify opens the restored `collection.anki2` for `PRAGMA integrity_check` and note-count compare, checks dest media file count against the source (excluding `media.trash`), requires dest file lengths to match the source, and fails if the newest `.colpkg` was planned but missing.
- VS Code L3 verify requires dest `install-extensions-<cli>.cmd` to list every source extension id and dest settings file lengths to match the source.
- SSH L3 verify requires dest file lengths to match the source, then runs `ssh -G localhost` against the restored `.ssh` folder when ssh.exe answers. Missing ssh skips the process check. Game-save cards also restore `Program Files (x86)\Steam\userdata` into `Saved Games\Steam userdata`, Steam library `steamapps\common` save folders, and LocalLow publisher / Steam-compatible folders. After a walker checkpoint, those Steam library folders and the known save/launcher folders come from the node index instead of listing every title or `DirectoryExists` on each known path. Firefox help documents `logins.db` next to `logins.json`. Thunderbird copies `logins.db` with `key4.db` the same way.
...
- Zotero, Calibre Library, Joplin, and Logseq graphs are recipe cards. Verify requires the catalog file (`zotero.sqlite`, `metadata.db`, `database.sqlite`, or `config.edn`). After a walker checkpoint, Zotero/Calibre/Joplin/Logseq folders come from the node index.
- High-value `*.vhdx` files are badged VM disk only when the first bytes are `vhdxfile`. Empty or text files named `.vhdx` stay unclassified. Other VM extensions (`*.vhd`, `*.vmdk`, `*.vdi`, `*.qcow2`, `*.ova`, `*.vbox`, `*.vmx`) still match by name.
...
- After a scan finishes, the live path caption is cleared so leftover “Classifying scanned files…” no longer looks like the app is still scanning. Stale walk/detect progress reports after the walk returns are ignored so that caption cannot come back.
- Opening last scan rebuilds High-value / Regeneratable counts, bytes, and per-rule Inspect lines from scan badges instead of showing 0 bytes.

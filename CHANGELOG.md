...
- Published `WinOldRecovery.exe` accepts `--scan <source> --report <file>` and records peak working set plus a 2,000-child Files-tree page. `tools/run-1m-scan.ps1` builds a 1,000×1,000 empty-file tree and launches that probe. The 1M published-EXE memory ceiling is not marked passed until that report says `Passed: true`.

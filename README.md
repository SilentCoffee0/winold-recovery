...
> **Status: Milestone 7 in progress.** Safety audit findings are in `BUG_AUDIT.md`. Remaining: interactive FlaUI, code signing, the manual VM matrix, a 1M-node published-EXE memory pass, kill-and-resume of the published EXE mid-copy, and other elevated/manual M0 checks in `docs/spikes/PENDING_MANUAL.md`. The 200 MB VHDX *preflight* disk-full probe passed; runtime mid-copy `ERROR_DISK_FULL` resume did not. The M0 tag has not been created. Green tests are not a safety sign-off.

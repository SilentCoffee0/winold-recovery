...
> **Status: Milestone 7 in progress.** Safety audit findings are in `BUG_AUDIT.md`. Remaining: code signing and the manual Windows 11 VM matrix (`cleanmgr` on a setup-created `Windows.old`, clean-VM startup measurement) in `docs/spikes/PENDING_MANUAL.md`. The elevated 100k FixtureGen self-check, 200 MB VHDX disk-full preflight **and** runtime pause/resume, published 1M-node `--scan` memory ceiling, published mid-copy kill-and-resume, the FlaUI smoke on the published EXE (including the Resume overlay), and Explorer long-path `/select,` passed. The M0 tag has not been created. Green tests are not a safety sign-off.

...
> **Status: Milestone 1 in progress.** Automated Milestone 0 coding, including GitHub Actions CI/release workflows, is in place. Source discovery and the `FileSystemWalker` scan engine are implemented. Elevated/manual M0 acceptance checks are still pending, so the M0 tag has not been created.

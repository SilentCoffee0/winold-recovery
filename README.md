...
> **Status: Milestone 7 in progress.** Safety audit findings are in `BUG_AUDIT.md`. Remaining: the elevated FlaUI scan→purge e2e (`tools/run-flaui-e2e.ps1`), code signing, and the manual Windows 11 VM matrix (`cleanmgr` on a setup-created `Windows.old`, clean-VM startup measurement) in `docs/spikes/PENDING_MANUAL.md`. The elevated 100k FixtureGen self-check, 200 MB VHDX disk-full preflight **and** runtime pause/resume, published 1M-node `--scan` memory ceiling, published mid-copy kill-and-resume, the FlaUI overlay smoke on the published EXE, and Explorer long-path `/select,` passed. The M0 tag has not been created. Green tests are not a safety sign-off.

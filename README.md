...
> **Status: Milestone 7 in progress.** Safety audit findings are in `BUG_AUDIT.md`. Remaining: interactive FlaUI, code signing, the manual VM matrix, a real 200 MB VHDX disk-full test, a 1M-node memory pass, and elevated/manual M0 checks. The M0 tag has not been created. Green tests are not a safety sign-off.

...
- **Complete:** Automated M0 coding through CI/release (M0 tag withheld). M1–M6. M7 slices: narrator names, redacted crash and handled-failure dialogs/status, contributor docs, ARM64 publish, skippable FlaUI smoke, first-run overlay, in-app local Help (no network), file-list type-ahead, page-size truncation test.

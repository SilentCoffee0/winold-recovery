...
- **Current item:** SignPath and the remaining Windows 11 VM matrix (`cleanmgr` on a setup-created `Windows.old`, clean-VM startup measurement). The interactive FlaUI smoke, 1M-node published-EXE memory ceiling, and Explorer long-path `/select,` passed 15 Sep 2026. Do not tag `v0.1.0-m0` or `v0.1.0` while the rest remain pending.
- **Complete:** Automated M0 coding through CI/release (M0 tag withheld). M1–M6. M7 slices: narrator names, redacted failures, Help/first-run, log viewer, compact layout, scan picker dates/Browse/Pause/skipped counters, ARM64 publish, restore percent/ETA, verify file-count report, Inspect owner SID, restore skip warnings, Space collapse, Ctrl+F search focus, Inspect mtime range and classification Why, purge token mint from session store, purge checklist (jobs/undecided/free space), purge cleanup-vs-direct radios, Delete Windows.old, cancel-during-purge with progress, Scan options (folder sizes + hash), recipe-card Inspect/Open/Restore/Leave Behind, grey reparse rows with ⊘ and problem explanations, decision chips and suggested-default tooltips, Files view radios, overview-card Inspect/Open/Restore/Leave Behind, Recent folder-group headers, Problems column explanations, system-color status strip, scan summary known-apps/high-value, live scan progress with profiles and file/folder counts, system message font/size for DPI and accessibility text size. M1 tree-scale tests: 100k-child expand under 300 ms, 1M synthetic SQLite children paged under 1.5 GB (schema v3 NOCASE parent index), and a 1,000,000-file on-disk walker scan under 60 s / 1.5 GB in the test process. I1 source watchdog on integration scans; I15 kill-process CopyTree of 50k files via RestoreHarness (published EXE kill still pending). Named I1–I16 tests, including I4/I9–I14 and window title with source path and step. Recipe L3 rows persist on the verify report so I10 consults the store. Git repos are detected anywhere under the profile (outermost card) with tree badges. Verify hashes VHDX past 64 MB; Git config restore includes ignore files. Analyze repositories runs git.exe through `IProcessRunner` and updates Git risk badges from porcelain output. Git L3 verify compares restored `HEAD` and, when git.exe is present, `rev-parse HEAD` plus porcelain status via `IProcessRunner` (`CollectLevel3Async`). Git detect parses refs/packed-refs/remotes/stash offline and leaves uncommitted/unpushed unknown until Analyze. Firefox cards report Primary Password from `key4.db` metadata (empty-password check; unknown if the file cannot be parsed). Analyze repositories copies `.git` to session temp and runs git against the copy (`--git-dir` / `--work-tree`) so Windows.old is not the git-dir. Git L3 verify copies source `.git` the same way; `-C` is only used on Recovered/. Git repos under `node_modules` / `.cache` are listed as vendored (no status analysis, default Leave Behind). Recipe detectors badge matching tree nodes (Firefox/Chrome/Edge, KeePass, GPG, WSL, Anki, Outlook, Thunderbird, VS Code, Terminal, Obsidian) and Syncthing folders as `[Syncthing folder: <label>]`. Firefox exports tabs from mozLz4 sessionstore and extensions with AMO links. Firefox discovery reads `profiles.ini` and `installs.ini` (empty profiles default to Leave Behind). Firefox L3 verify checks recovered `profiles.ini`, `places.sqlite` integrity and bookmark count, and the key4/logins pair. Firefox transplant copies session/bookmark backups, extensions, and site storage (lock files skipped). Preview Edit mapping persists Syncthing folder path overrides into the rewritten paused config. L2 SHA-256 samples 2 % of large files (min 200), always hashes sensitive nodes, and hashes everything when strong verify was enabled at scan.
- **Remaining M0 (explicitly pending, not claimed passed):** clean-Windows-11 elevated WPF launch and cold/warm startup on a clean image; real-VM `cleanmgr` handler on a setup-created `Windows.old`; SignPath. See `docs/spikes/PENDING_MANUAL.md`.
...
- **Next exact implementation step:** the remaining gates need a disposable Windows 11 VM (`cleanmgr` on a setup-created `Windows.old`, cold/warm startup on a clean image) or signing credentials. Do not tag `v0.1.0-m0`.

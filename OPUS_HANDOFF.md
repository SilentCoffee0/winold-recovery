...
- **Current item:** SignPath and the manual Windows 11 VM matrix (`cleanmgr` on a setup-created `Windows.old`, clean-VM startup measurement, Explorer long-path selection). The interactive FlaUI smoke and the 1M-node published-EXE memory ceiling passed 15 Sep 2026. Do not tag `v0.1.0-m0` or `v0.1.0` while the rest remain pending.
...
- **Next exact implementation step:** the remaining gates need a disposable Windows 11 VM (`cleanmgr` on a setup-created `Windows.old`, cold/warm startup on a clean image, Explorer long-path click) or signing credentials. Do not tag `v0.1.0-m0`.

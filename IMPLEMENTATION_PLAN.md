...
- M7 second slice complete on 13 Sep 2026. Scan/restore/verify/purge failures surface a redacted explanation and session log path; first-run promises text; file-list type-ahead; narrator names on action buttons; NodeBrowser page-size truncation test. Interactive FlaUI, SignPath, the VM matrix, 1M-node memory, and elevated/manual M0 checks remain pending. Do not tag `v0.1.0` or `v0.1.0-m0` until `docs/spikes/PENDING_MANUAL.md` and `docs/manual-test-matrix.md` pass.

...
- Recipe L3 persist complete on 14 Sep 2026. `RecipeHost.CollectLevel3` verifies executed writes (not a re-plan that would Keep-Both existing dests) and inserts level-3 rows on the L0–L2 report. A failed L3 check fails `LastVerifyReportAllOk`, so I10 `verify-store` blocks purge even when UI `VerifyAllOk` is true. Do not tag.

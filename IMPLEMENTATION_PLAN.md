...
- M3 item 1 (PlanBuilder): first slice complete on 13 Sep 2026. `PlanBuilder` emits `CopyTree` for uniform Restore directories and `CopyFile` for mixed trees, refuses destination/source containment (I13), never emits whole `AppData`/`Local`/`Roaming`/`LocalLow` plan items (I4), and persists items in the session database. `GetDiskFreeSpaceExW` and `MoveFileExW` are available for the copy engine. Preflight, CopyEngine, journal resume, Verifier, and Preview UI remain.

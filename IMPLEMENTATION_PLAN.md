...
- M1 item 5: first slice complete on 13 Sep 2026. The WPF shell shows the six-step bar, source-integrity status strip, Scan source list, and a virtualized Decide files list backed by `NodeBrowser` (paged children, largest/recent/search/unknown/problems queries). Scan runs `ScanOrchestrator` (profiles then walker). Open Folder uses `IProcessRunner` with `explorer.exe /select,`. Cards view, on-window view-mode chrome, context menus, and FlaUI smoke remain.

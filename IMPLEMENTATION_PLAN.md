...
- M7 third slice complete on 13 Sep 2026. First-run overlay (six steps + two promises); in-app Help from bundled local Markdown (`docs/help` + limitations), F1, Log/Help status buttons; help paths cannot traverse out of the help folder and Markdown links drop their URL targets (I7). Interactive FlaUI, SignPath, the VM matrix, 1M-node memory, and elevated/manual M0 checks remain pending. Do not tag `v0.1.0` or `v0.1.0-m0` until `docs/spikes/PENDING_MANUAL.md` and `docs/manual-test-matrix.md` pass.

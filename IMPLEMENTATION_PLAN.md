...
- M1 item 5: implementation complete on 13 Sep 2026 except interactive FlaUI smoke (no display in this environment; still pending). The WPF shell shows the six-step bar, source-integrity status strip, Scan source list, Decide Cards for profiles and standard folders, Files view-mode chrome (Tree/Largest/Recent/Search/Unknown/Problems), a virtualized list with badges and an extended-selection context menu (Inspect, Open Folder, Restore, Leave Behind, Undecided, Copy path), and an Inspect pane. Scan runs `ScanOrchestrator` (profiles then walker). Open Folder uses `IProcessRunner` with `explorer.exe /select,`.

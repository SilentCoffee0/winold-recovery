...
- M6 first slice complete on 13 Sep 2026. Purge is unreachable until verify succeeds. `PurgeAuthorization` mints a `PurgeToken` only when every I10 gate passes (verify, files checked, undecided acknowledged, typed folder name, custom-root confirmation, no restore job, executable/session/destination not inside the source). `PurgeExecutor` records `cleanmgr.exe /sagerun:777` then falls back to bottom-up delete that never follows reparse points. Session `purge-manifest.txt` and a support-bundle zip are written outside the source. Real-VM cleanup-handler deletion remains a pending manual check. 10-day warning shows Setup Cleanup task presence and next-run time from `schtasks /Query` LIST output.

...
- Remaining native APIs listed in M0 work item 2 (`GetDiskFreeSpaceExW`, `SetFileTime`, and `MoveFileExW`) are not yet implemented; they will be added before the copy engine that requires them. `GetFileInformationByHandleEx` (attribute tag) and `FSCTL_GET_REPARSE_POINT` shipped with the walker.
...
- M7 twelfth slice complete on 14 Sep 2026. Decide Undo (Ctrl+Z) uses the 50-step stack; recipe component Restore/Leave Behind persist in session kv and feed PlanCard; purge writes `session-record.txt` in the session folder before delete. Do not tag.
